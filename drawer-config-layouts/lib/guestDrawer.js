 ...
 */
 export default (api) => {
  ...
  const wasmApi = WebAssembly.instantiateStreaming(
    fetch('https://plugins.coviu.com/examples/wasm/test.wasm')
  ...
  const calculate = () =>
    Promise.all([wasmApi, mesh]).then(([{ squarer }, calculations]) => {
      ...
      return thunk(DRAWER_ID, { drawerState }, () => {
        ...
        return h('div.Drawer_Content', [
          ...
          h(`div.${styles.card}`, [
            ...
            output
              ? h('p', `[Using WebAssembly] ${input} * ${input} = ${output}`)
              : undefined,
          ]),
        ]);
      });
